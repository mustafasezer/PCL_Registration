...
int main()
{
    ...
    pcl_tool.MAX_CORRESPONDENCE_INIT = 1.0;
    pcl_tool.MAX_CORRESPONDENCE_STEP = 0.1;

    std::vector<float> steps;
    steps.push_back(0.075);
    steps.push_back(0.05);
    steps.push_back(0.025);

    std::vector<int> ground_vec;
    ground_vec.push_back(0);
    ground_vec.push_back(1);


    for(int ground=0; ground<ground_vec.size(); ground++){
        for(int config=0; config<steps.size(); config++){
            pcl_tool.MAX_CORRESPONDENCE_STEP = steps[config];
            fstream icp_result, ndt_result;
            stringstream icp_result_filename;
            icp_result_filename << "/home/mustafasezer/icp_results_g" << ground << "_" << config << ".txt";
            string icp_result_filename_str = icp_result_filename.str();
            icp_result.open(icp_result_filename_str.c_str(), ios::out);
            ...
            for(int i=1; i<=27; i++){
                ...
                pcl::PointCloud<PointT>::Ptr cloud_in_transformed_filtered (new pcl::PointCloud<PointT>);
                pcl::PointCloud<PointT>::Ptr cloud_targ_filtered (new pcl::PointCloud<PointT>);
                if(ground==0){
                    cloud_in_transformed_filtered = pcl_tool.getSlice(cloud_in_transformed, 2.5, 20, false);
                    cloud_targ_filtered = pcl_tool.getSlice(cloud_targ, 2.5, 20, false);
                }
                else{
                    cloud_in_transformed_filtered = pcl_tool.getSlice(cloud_in_transformed, 0.2, 2.5, true);
                    cloud_targ_filtered = pcl_tool.getSlice(cloud_targ, 0.2, 2.5, true);
                }
